  ...
  <div>
    ...
    <div class="content table-responsive table-full-width">
        <table class="table table-hover table-striped">
            ...
            <tbody>
              <tr>
                <td  class="text-center" v-for="(sc, i) in stockcategories" :key="i" @click="setCategory(sc.name, sc.id)"
                :class="catClick">
                  <span class="cursor">
                    {{sc.name}}
                  </span>
        ...
        <hr>
        <div v-if="this.category.name">
          <h3 class="text-center">{{this.category.name}}</h3>
          <div>
            ...
            <div class="content table-responsive table-full-width">
              <table class="table table-hover table-striped" style="margin-bottom: 0px">
                ...
                <tbody>
                  <tr>
                    <td  class="text-center" v-for="(sc, i) in types" :key="i" @click="setType(sc.name, sc.id)">
                      <span class="cursor">
                        {{sc.name}}
                      </span>
                    </td>
                      ...
                      <tbody>
                        <tr>
                          ...
                          <td style="display: flex; align-items: center;">
                            <select class="form-control form-control-sm" v-model="country">
                              <option v-for="(ft, i) in countries" :key="i" :value="ft.id">{{ft.name}}</option>
                            </select>
                            <a class="btn btn-sm btn-primary" @click="getCompetition">get</a>
                          </td>
                          <td class="text-center" >
                            <div style="display: flex; align-items: center;" v-if="this.competitions">
                              <select class="form-control form-control-sm" v-model="competition">
                                <option v-for="(ft, i) in competitions" :key="i" :value="ft.id">{{ft.name}}</option>
                              </select>
...
<script>
  export default {
    ...
    methods: {
      getEvent() {
        this.events = null;
        const country_id = this.country;
        const sport_id = this.sport;
        const competition_id = this.competition;
        const that =  this
        axios.get(`/api/stock/events/${competition_id}`)
        .then((res) => {
          if(res.data.length) {
      ...
      getCompetition() {
        ...
        const sport_id = this.sport;
        const that =  this
        axios.get(`/api/stock/competitions/${country_id}/${sport_id}`)
        .then((res) => {
          that.competitions = res.data;
          that.competition = res.data[0].id;
        }).catch((e) => {
          alert('an error must have occured try again');
        })
      },
